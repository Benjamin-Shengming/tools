...
void setup_log(int log_verbosity)
{
    // syslog always has full details
    auto syslog_sink = std::make_shared<spdlog::sinks::syslog_sink_mt>("ql_sign", 0, LOG_USER, false);
    ...
    spdlog::set_default_logger(std::make_shared<spdlog::logger>("ql_sign", sink_list));
