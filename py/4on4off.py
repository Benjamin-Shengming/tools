    ...
    for week in month_calendar:
        for day in week:
            if day == 0:
              ...
              _, d_str, _ = cl(datetime(year, month, day), True if day in days else False)
...
def parse_args(): 
    ...
    parser.add_argument('--end_year', type=int, default=current_year, help='Check year')
    parser.add_argument('--end_month', type=int, default=current_month, help='Check month')
    parser.add_argument('--end_day', type=int, default=current_day, help='Check day')
    parser.add_argument('--one_day', action='store_true', default=False, help='check on day only')
    parser.add_argument('--show_list', action='store_true', default=False, help='show as calendar format')
    ...
    if (start_date > end_date): 
        raise  RuntimeError(f"End date {end_date.strftime('%Y-%m-%d')}  is before start date {start_date.strftime('%Y-%m-%d')}")
